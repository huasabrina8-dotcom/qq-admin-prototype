...
4. 抽奖活动的**规则**以 `qq-lottery-prototype` 已定稿需求为准（循环重置、分享任务添加才展示、任务1关闭走红包雨、奖品挂所属排名、保底）。现已按该规则 + UAT 八张截图落到本仓库 `#/activity` 编辑活动，不要另写第二套发奖公式。前台抽奖页仍在抽奖项目。
...
4b. **活动管理**（`#/activity`）以 UAT 截图为准：筛类型/状态/悬浮开关/创建时间；工具栏 +新增、删除、标签管理；列含双端图标、选择标签、排序 −/+、状态开关、悬浮或 `--`、行编辑+删除。点标签管理弹出 **新增/管理** 芯片弹层。点 +新增/编辑打开 **编辑活动** 宽抽屉。活动类型 23 项按截图顺序；当日首存四个步进器；周亏损返现按三张截图（图标、最高返现、打码倍数、选游戏、VIP 比例表、额外周亏损开关/比例/星期、用户总输）；日亏损返现按两张截图（同上但无额外周亏损，多救援金开关，总输 ? 为日输）；金钱如雨按三张截图（图标、参与充值/每场奖励/最高获得、打码倍数、每日/额外 Tab、时段表、下雨 VIP 表与均分奖金）；转盘奖励按两张截图（图标、奖励金额、初始金额 P~P、周期天、随机比例/次数、任务1输额+游戏+展开明细、任务2邀请/三方、简讯模板）；抽奖活动按八张截图 + 抽奖项目定稿规则（开始时间、循环周期、分享任务、英文/菲语图视频、可选任务、红包雨、VIP、流水排名 top、奖品、保底、分享链接/官方账号）。不要用同一套表单冒充全类型。细则见 PRD **2.16 Locked**。
